...
def load_CUEBIQMI_crv(filename, all_dates, all_counties):
  with open(filename, 'r', errors='ignore') as fin_raw:
    fin = csv.reader(fin_raw, dialect=csv.excel)
    headers = fin.__next__()
    header_ndx = {h: i for i, h in enumerate(headers)}
    ...
    for line_ndx in fin:
      date = dt.strptime(line_ndx[header_ndx['date']], '%Y-%m-%d').date()
      county_str = line_ndx[header_ndx['countyFIPS']]
      # state_str = line_ndx[header_ndx['statefp10']]
      if (county_str) not in parsed_county_ids and int(county_str) in df_data:
        while len(county_str) < 3:
          county_str = '0' + county_str
        # county = int(state_str + county_str)
        county = int(county_str)
        parsed_county_ids[county_str] = county
      
      if county_str in parsed_county_ids and is_number(line_ndx[header_ndx['CMI']]):
        county = parsed_county_ids[county_str]
        # print(line_ndx[header_ndx['CMI']])
        CMI = float(line_ndx[header_ndx['CMI']])
        df_data[county][date] = CMI
    
    df = pd.DataFrame.from_dict(df_data, orient='index', columns=all_dates)
    df.fillna(-1, inplace=True)
    return df

def load_googlemobility_crv(filename, all_dates, all_counties):
  with open(filename, 'r', errors='ignore') as fin_raw:
    fin = csv.reader(fin_raw, dialect=csv.excel)
    headers = fin.__next__()
    header_ndx = {h: i for i, h in enumerate(headers)}
    ...
    for line_ndx in fin:
      date = dt.strptime(line_ndx[header_ndx['date']], '%Y-%m-%d').date()
      county_str = line_ndx[header_ndx['county_fips_code']]
      # state_str = line_ndx[header_ndx['statefp10']]
      if (county_str) not in parsed_county_ids and int(county_str) in df_data:
        while len(county_str) < 3:
          county_str = '0' + county_str
      ...
      if county_str in parsed_county_ids and is_number(line_ndx[header_ndx['retail_rec']]) and is_number(line_ndx[header_ndx['groc_pharm']]) and is_number(line_ndx[header_ndx['parks']]) and is_number(line_ndx[header_ndx['transit']]) and is_number(line_ndx[header_ndx['work']]) and is_number(line_ndx[header_ndx['residential']]):
        county = parsed_county_ids[county_str]
        # print(line_ndx[header_ndx['CMI']])
        retail_rec = float(line_ndx[header_ndx['retail_rec']])
        groc_pharm = float(line_ndx[header_ndx['groc_pharm']])
        parks = float(line_ndx[header_ndx['parks']])
        transit = float(line_ndx[header_ndx['transit']])
        work = float(line_ndx[header_ndx['work']])
        residential = float(line_ndx[header_ndx['residential']])
        df_data[county][date] = [retail_rec, groc_pharm, parks, transit, work, residential]
    df = pd.DataFrame.from_dict(df_data, orient='index', columns=all_dates)
    df.fillna(-1, inplace=True)
    return df
...
def get_all_datasets():
  ...
  cuebiqMI = load_CUEBIQMI_crv(filename='data/processedCUEBIQdata.csv',
    all_dates=confirmed_cases_df_init.columns,
    all_counties=confirmed_cases_df_init.index)

  googleMobility = load_googlemobility_crv(filename='data/processedGoogleData.csv',
    all_dates=confirmed_cases_df_init.columns,
    all_counties=confirmed_cases_df_init.index)
